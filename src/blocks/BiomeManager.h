...
typedef vector<BiomeHeightBoundary*> BiomeHeightBoundaryList;
...
public:
	...
	void UpdatePlanes(const Matrix4x4& transformationMatrix);
};

typedef vector<ZoneData*> ZoneDataList;
...
public:
	...
	void AddTown(const vec3& townCenter, float radius);
	void AddTown(const vec3& townCenter, float length, float height, float width);
	void AddSafeZone(const vec3& safeZoneCenter, float radius);
	void AddSafeZone(const vec3& safeZoneCenter, float length, float height, float width);

	// Get biome
	Biome GetBiome(const vec3& position) const;

	// Town
	bool IsInTown(const vec3& position, ZoneData **pReturnTown);
	float GetTowMultiplier(const vec3& position);

	// Safe zone
	bool IsInSafeZone(const vec3& position, ZoneData **pReturnSafeZone);
