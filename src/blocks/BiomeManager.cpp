...
//printf("vvb BiomeManager pushback biome = %d\n", biome);
	m_vpBiomeHeightBoundaryList[biome].push_back(pNewBiomeHeightBoundary);
}

void BiomeManager::AddTown(const vec3& townCenter, float radius)
{
...
void BiomeManager::AddTown(const vec3& townCenter, float length, float height, float width)
{
	ZoneData* pNewTown = new ZoneData();
	pNewTown->m_origin = townCenter;
...
void BiomeManager::AddSafeZone(const vec3& safeZoneCenter, float radius)
{
...
void BiomeManager::AddSafeZone(const vec3& safeZoneCenter, float length, float height, float width)
{
	ZoneData* pNewSafeZone = new ZoneData();
	pNewSafeZone->m_origin = safeZoneCenter;
...
// Get biome
Biome BiomeManager::GetBiome(const vec3& position) const
{
...
// Town
bool BiomeManager::IsInTown(const vec3& position, ZoneData **pReturnTown)
{
	for (unsigned int i = 0; i < m_vpTownsList.size(); i++)
	{
		...
		else if (pTown->m_regionType == BiomeRegionType_Cube)
		{
			...
			for (const Plane3D& plane: pTown->m_planes)
			{
				distance = plane.GetPointDistance(position - pTown->m_origin);
...
float BiomeManager::GetTowMultiplier(const vec3& position)
{
...
// Safe zone
bool BiomeManager::IsInSafeZone(const vec3& position, ZoneData **pReturnSafeZone)
{
	for (unsigned int i = 0; i < m_vpSafeZonesList.size(); i++)
	{
		...
		else if (pSafeZone->m_regionType == BiomeRegionType_Cube)
		{
			...
			for (const Plane3D& plane: pSafeZone->m_planes)
			{
				distance = plane.GetPointDistance(position - pSafeZone->m_origin);
...
//printf("vvb biome = %d (%f, %f, %f)\n", biome, xPos, yPos, zPos);
if (biome == 0) return;
	float red1 = 0.0f;
	float green1 = 0.0f;
	float blue1 = 0.0f;
...
void ZoneData::UpdatePlanes(const Matrix4x4& transformationMatrix)
{
