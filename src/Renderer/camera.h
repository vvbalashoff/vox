...
public:
	...
	void SetZoomAmount(float amount) { m_zoomAmount = amount; }
	const vec3& GetPosition() const { return m_position; }
	const vec3& GetFakePosition() const { return m_fakePosition; }
	vec3 GetFacing()const { return vec3(m_facing.x, m_facing.y, m_facing.z); }
	const vec3& GetUp() const { return m_up; }
	const vec3& GetRight() const { return m_right; }
	vec3 GetView() const { return (m_position + (m_facing*m_zoomAmount)); }
