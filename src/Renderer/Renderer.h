...
public:
	...
	// Screen projection
	vec3 GetWorldProjectionFromScreenCoordinates(int x, int y, float z) const;
	void GetScreenCoordinatesFromWorldPosition(vec3 pos, int *x, int *y) const;
	...
	static GLenum GetBlendEnum(BlendFunction flag);
	...
	Colour GetLightAmbient(unsigned int id) const;
	Colour GetLightDiffuse(unsigned int id) const;
	Colour GetLightSpecular(unsigned int id) const;
	vec3 GetLightPosition(unsigned int id) const;
	float GetConstantAttenuation(unsigned int id) const;
	float GetLinearAttenuation(unsigned int id) const;
	float GetQuadraticAttenuation(unsigned int id) const;
	...
	// Cube textures
	bool LoadCubeTexture(int *width, int *height, string front, string back, string top, string bottom, string left, string right, unsigned int *pID) const;
	...
	static unsigned int GetStride(VertexType type);

	// Mesh
	static OpenGLTriangleMesh* CreateMesh(OGLMeshType meshType);
	void ClearMesh(OpenGLTriangleMesh* pMesh);
	static unsigned int AddVertexToMesh(const vec3& p, const vec3& n, float r, float g, float b, float a, OpenGLTriangleMesh* pMesh);
	static unsigned int AddTextureCoordinatesToMesh(float s, float t, OpenGLTriangleMesh* pMesh);
	static unsigned int AddTriangleToMesh(unsigned int vertexId1, unsigned int vertexId2, unsigned int vertexId3, OpenGLTriangleMesh* pMesh);
	...
	static void GetMeshInformation(int *numVerts, int *numTris, OpenGLTriangleMesh* pMesh);
	...
	int  GetPickedObject() const;

	// Frustum
	Frustum* GetFrustum(unsigned int frustumid) const;
	int PointInFrustum(unsigned int frustumid, const vec3 &point) const;
	int SphereInFrustum(unsigned int frustumid, const vec3 &point, float radius) const;
	int CubeInFrustum(unsigned int frustumid, const vec3 &center, float x, float y, float z) const;
	...
	FrameBuffer* GetFrameBuffer(string name) const;
	FrameBuffer* GetFrameBuffer(int index) const;
	int GetFrameBufferIndex(string name) const;
	void StartRenderingToFrameBuffer(unsigned int frameBufferId);
	void StopRenderingToFrameBuffer(unsigned int frameBufferId);
	unsigned int GetDiffuseTextureFromFrameBuffer(unsigned int frameBufferId) const;
	unsigned int GetPositionTextureFromFrameBuffer(unsigned int frameBufferId) const;
	unsigned int GetNormalTextureFromFrameBuffer(unsigned int frameBufferId) const;
	unsigned int GetDepthTextureFromFrameBuffer(unsigned int frameBufferId) const;
