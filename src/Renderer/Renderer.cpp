...
void Renderer::SetClearColour(const Colour& col)
{
...
// Screen projection
vec3 Renderer::GetWorldProjectionFromScreenCoordinates(int x, int y, float z) const
{
	double mvmatrix[16];
...
void Renderer::GetScreenCoordinatesFromWorldPosition(vec3 pos, int *x, int *y) const
{
...
Colour Renderer::GetLightAmbient(unsigned int id) const
{
	return m_lights[id]->Ambient();
}

Colour Renderer::GetLightDiffuse(unsigned int id) const
{
	return m_lights[id]->Diffuse();
}

Colour Renderer::GetLightSpecular(unsigned int id) const
{
	return m_lights[id]->Specular();
}

vec3 Renderer::GetLightPosition(unsigned int id) const
{
	return m_lights[id]->Position();
}

float Renderer::GetConstantAttenuation(unsigned int id) const
{
	return m_lights[id]->ConstantAttenuation();
}

float Renderer::GetLinearAttenuation(unsigned int id) const
{
	return m_lights[id]->LinearAttenuation();
}

float Renderer::GetQuadraticAttenuation(unsigned int id) const
{
...
// Cube textures
bool Renderer::LoadCubeTexture(int *width, int *height, string front, string back, string top, string bottom, string left, string right, unsigned int *pID) const
{
	bool loaded = false;
...
unsigned int Renderer::AddVertexToMesh(const vec3& p, const vec3& n, float r, float g, float b, float a, OpenGLTriangleMesh* pMesh)
{
...
int Renderer::GetPickedObject() const
{
	int objectsFound = objectsFound = glRenderMode(GL_RENDER);
	if (objectsFound > 0)
	{
...
// Frustum
Frustum* Renderer::GetFrustum(unsigned int frustumid) const
{
...
int Renderer::PointInFrustum(unsigned int frustumid, const vec3 &point) const
{
	Frustum* pFrustum = m_frustums[frustumid];
...
int Renderer::SphereInFrustum(unsigned int frustumid, const vec3 &point, float radius) const
{
	Frustum* pFrustum = m_frustums[frustumid];
...
int Renderer::CubeInFrustum(unsigned int frustumid, const vec3 &center, float x, float y, float z) const
{
	Frustum* pFrustum = m_frustums[frustumid];
...
FrameBuffer* Renderer::GetFrameBuffer(string name) const
{
	int foundIndex = -1;
...
FrameBuffer* Renderer::GetFrameBuffer(int index) const
{
	return m_vFrameBuffers[index];
}

int Renderer::GetFrameBufferIndex(string name) const
{
	int foundIndex = -1;
	for (int i = 0; i < (int)m_vFrameBuffers.size(); i++)
	{
...
unsigned int Renderer::GetDiffuseTextureFromFrameBuffer(unsigned int frameBufferId) const
{
	return m_vFrameBuffers[frameBufferId]->m_diffuseTexture;
}

unsigned int Renderer::GetPositionTextureFromFrameBuffer(unsigned int frameBufferId) const
{
	return m_vFrameBuffers[frameBufferId]->m_positionTexture;
}

unsigned int Renderer::GetNormalTextureFromFrameBuffer(unsigned int frameBufferId) const
{
	return m_vFrameBuffers[frameBufferId]->m_normalTexture;
}

unsigned int Renderer::GetDepthTextureFromFrameBuffer(unsigned int frameBufferId) const
{
...
{
	// Load the shader
	glShader* lpShader = ShaderManager.loadfromFile(vertexFile, fragmentFile);  // load (and compile, link) from file
	if (lpShader != nullptr)
	{
		// Push the vertex array onto the list
