...
string Player::GetModelName() const
{
	return m_modelName;
}

void Player::SetRespawnPosition(const vec3& pos)
{
	m_respawnPosition = pos;
}

const vec3& Player::GetRespawnPosition() const
{
	return m_respawnPosition;
}

vec3 Player::GetCenter() const
{
...
const vec3& Player::GetForwardVector() const
{
	return m_forward;
}

const vec3& Player::GetRightVector() const
{
	return m_right;
}

const vec3& Player::GetUpVector() const
{
	return m_up;
}
...
void Player::SetForwardVector(const vec3& forward)
{
...
// Unloading
void Player::LoadWeapon(bool left, string weaponFile) const
{
	if (left)
	{
...
void Player::EquipItem(InventoryItem* pItem, bool supressAudio)
{
	...
	if (supressAudio == false)
	{
		if (l_bEquipWeapon)
		{
			...
			VoxGame::GetInstance()->PlaySoundEffect(eSoundEffect_EquipSword, m_position, 1.5f);
		}
		else
		{
			// vvb
			// VoxGame::GetInstance()->PlaySoundEffect(eSoundEffect_EquipCloth);
			VoxGame::GetInstance()->PlaySoundEffect(eSoundEffect_EquipCloth, m_position, 1.5f);
		}
	}
...
// Collision
bool Player::CheckCollisions(const vec3& positionCheck, const vec3& previousPosition, vec3 *pNormal, vec3 *pMovement, bool *pStepUpBlock) const
{
...
// Selection
bool Player::GetSelectionBlock(vec3 *blockPos, int* chunkIndex, int* blockX, int* blockY, int* blockZ) const
{
	float distance = 0.0f;
	bool collides = false;
	int interations = 0;
...
bool Player::GetPlacementBlock(vec3 *blockPos, int* chunkIndex, int* blockX, int* blockY, int* blockZ) const
{
	float distance = 0.0f;
	bool collides = false;
	int interations = 0;
...
Chunk* Player::GetCachedGridChunkOrFromPosition(const vec3& pos) const
{
...
// Camera
void Player::SetCameraPosition(const vec3& cameraPos)
{
	m_cameraPosition = cameraPos;
}

void Player::SetCameraForward(const vec3& cameraForward)
{
	m_cameraForward = cameraForward;
}

void Player::SetCameraUp(const vec3& up)
{
	m_cameraUp = up;
}

void Player::SetCameraRight(const vec3& right)
{
	m_cameraRight = right;
}

// Loading configuration and settings for the game
void Player::LoadCharacterSettings() const
{
...
// Movement
const vec3& Player::GetPositionMovementAmount() const
{
...
void Player::SetMoveToTargetPosition(const vec3& pos)
{
...
void Player::SetLookAtTargetAfterMoveToPosition(const vec3& lookAt)
{
...
string Player::GetStrengthModifierString() const
{
...
string Player::GetDexterityModifierString() const
{
	char returnString[64];
	char signModifier[2];
	char colourModifier[16];
...
string Player::GetIntelligenceModifierString() const
{
	char returnString[64];
	char signModifier[2];
	char colourModifier[16];
...
string Player::GetVitalityModifierString() const
{
	char returnString[64];
	char signModifier[2];
	char colourModifier[16];
...
string Player::GetArmorModifierString() const
{
	char returnString[64];
	char signModifier[2];
	char colourModifier[16];
...
string Player::GetLuckModifierString() const
{
	char returnString[64];
	char signModifier[2];
	char colourModifier[16];
...
// Looking
void Player::LookAtPoint(const vec3& point)
{
	m_bLookAtPoint = true;
	m_lookPoint = point;
}
...
bool Player::Is2HandedSword() const
{
