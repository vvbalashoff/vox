...
void Matrix4x4::SetTranslation(const vec3& trans) {
	//LoadIdentity();
...
void Matrix4x4::SetScale(const vec3& scale)
{
	m[0] = scale.x;
	m[5] = scale.y;
	m[10] = scale.z;
...
Matrix4x4 Matrix4x4::GetNegative() const {
...
Matrix4x4 Matrix4x4::GetOrthoNormal() const {
...
vec3 Matrix4x4::GetRightVector() const
{
	return vec3(m[0], m[1], m[2]);
}

vec3 Matrix4x4::GetUpVector() const
{
	return vec3(m[4], m[5], m[6]);
}

vec3 Matrix4x4::GetForwardVector() const
{
