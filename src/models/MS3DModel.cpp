...
bool MS3DModel::LoadModel(const char *modelFileName, bool lStatic)
{
	...
	for ( i = 0; i < nGroups; i++ )
	{
		...
		word triangles = *( word* )pPtr;
		pPtr += sizeof( word );
		int *pTriangleIndices = new int[triangles];
		...
		pMeshes[i].numTriangles = triangles;
...
void MS3DModel::Render(bool lMesh, bool lNormals, bool lBones, bool lBoundingBox) const
{
	if(lMesh)
	{
		if(mbStatic)
...
void MS3DModel::RenderMesh_Static() const
{
...
void MS3DModel::RenderNormals() const
{
...
void MS3DModel::RenderBoundingBox() const
{
	mpRenderer->PushMatrix();
