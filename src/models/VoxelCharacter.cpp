...
static Matrix4x4 emptyMatrix4x4;
...
const vec3& VoxelCharacter::GetBoneScale() const
{
	return m_boneScale;
}
...
void VoxelCharacter::SetMeshSingleColour(float r, float g, float b) const
{
	if(m_pVoxelModel)
	{
...
{
	const char *pBoneName = m_pCharacterAnimator[AnimationSections_FullBody]->GetModel()->GetNameFromBoneIndex(boneIndex);
	if(strcmp(pBoneName, "Head") == 0)
	{
		return m_breathingBodyYOffset * 0.75f;
	}
	if(strcmp(pBoneName, "Body") == 0)
	{
		return m_breathingBodyYOffset;
	}
	if(strcmp(pBoneName, "Legs") == 0)
	{
		return m_breathingBodyYOffset * 0.5f;
	}
	if(strcmp(pBoneName, "Right_Shoulder") == 0)
	{
		return m_breathingHandsYOffset;
	}
	if(strcmp(pBoneName, "Left_Shoulder") == 0)
	{
		return m_breathingHandsYOffset;
	}
	if(strcmp(pBoneName, "Right_Hand") == 0)
	{
		return m_breathingHandsYOffset;
	}
	if(strcmp(pBoneName, "Left_Hand") == 0)
	{
		return m_breathingHandsYOffset;
	}
...
const vec3& VoxelCharacter::GetEyesOffset() const
{
	return m_eyesOffset;
}

const vec3& VoxelCharacter::GetMouthOffset() const
{
	return m_mouthOffset;
}

void VoxelCharacter::SetEyesOffset(const vec3& offset)
{
	m_eyesOffset = offset;
}

void VoxelCharacter::SetMouthOffset(const vec3& offset)
{
...
// Face looking
const vec3& VoxelCharacter::GetFaceLookingDirection() const
{
	return m_faceLookingDirection;
}

void VoxelCharacter::SetFaceLookingDirection(const vec3& looking)
{
...
const vec3& VoxelCharacter::GetFaceTargetDirection() const
{
	return m_faceTargetDirection;
}

void VoxelCharacter::SetFaceTargetDirection(const vec3& target)
{
...
void VoxelCharacter::PlayAnimationOnPaperDoll(const char *lAnimationName, bool left) const
{
	if (left)
	{
...
void VoxelCharacter::SetWeaponTrailsOriginMatrix(float dt, const Matrix4x4& originMatrix) const
{
	if(m_pLeftWeapon != NULL)
	{
		if(m_leftWeaponLoaded)
...
// Rendering
void VoxelCharacter::Render(bool renderOutline, bool reflection, bool silhouette, const Colour& OutlineColour, bool subSelectionNamePicking)
{
	if(m_pVoxelModel != NULL)
	{
...
void VoxelCharacter::RenderSubSelection(string subSelection, bool renderOutline, bool silhouette, const Colour& OutlineColour)
{
	if(m_pVoxelModel != NULL)
	{
		m_pRenderer->PushMatrix();
...
void VoxelCharacter::RenderWeapons(bool renderOutline, bool reflection, bool silhouette, const Colour& OutlineColour) const
{
	if(m_pLeftWeapon != NULL)
	{
		if(m_leftWeaponLoaded)
