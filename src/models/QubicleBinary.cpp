...
const Matrix4x4& QubicleBinary::GetModelMatrix(int qubicleMatrixIndex)
{
	if(m_loaded == false)
	{
		return dummyMatrix;
...
#ifdef _WIN32
	if (fopen_s(&pQBfile, qbFilename, "rb"))
		return false;
#else
	pQBfile = fopen(qbFilename, "rb");
	if (pQBfile == nullptr)
		return false;
#endif //_WIN32	
...
void QubicleBinary::GetColour(int matrixIndex, int x, int y, int z, float* r, float* g, float* b, float* a) const
{
	...
	{
		const QubicleMatrix* pMatrix = m_vpMatrices[matrixIndex];
...
QubicleMatrix* QubicleBinary::GetQubicleMatrix(int index) const
{
	return m_vpMatrices[index];
}

QubicleMatrix* QubicleBinary::GetQubicleMatrix(const char* matrixName) const
{
	for(unsigned int i = 0; i < m_numMatrices; i++)
	{
		if(strcmp(m_vpMatrices[i]->m_name, matrixName) == 0)
...
//Rendering
void QubicleBinary::Render(bool renderOutline, bool reflection, bool silhouette, const Colour& OutlineColour)
{
...
void QubicleBinary::RenderWithAnimator(MS3DAnimator** pSkeleton, VoxelCharacter* pVoxelCharacter, bool renderOutline, bool reflection, bool silhouette, const Colour& OutlineColour, bool subSelectionNamePicking)
{
...
void QubicleBinary::RenderSingleMatrix(MS3DAnimator** pSkeleton, VoxelCharacter* pVoxelCharacter, string matrixName, bool renderOutline, bool silhouette, const Colour& OutlineColour)
{
	if(pVoxelCharacter == nullptr)
	{
		return;
