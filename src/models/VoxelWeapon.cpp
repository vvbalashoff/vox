...
void VoxelWeapon::SetRenderOffset(const vec3& offset)
{
	m_renderOffset = offset;
}

const vec3& VoxelWeapon::GetRenderOffset() const
{
...
vec3 VoxelWeapon::GetCenter() const
{
	vec3 centerPos;
	for(int i = 0; i < m_numAnimatedSections; i++)
	{
...
void VoxelWeapon::GetLightParams(int lightIndex, unsigned int *lightId, vec3 *position, float *radius, float *diffuseMultiplier, Colour *colour, bool *connectedToSegment) const
{
...
void VoxelWeapon::GetParticleEffectParams(int particleEffectIndex, unsigned int *particleEffectId, vec3 *position, string* name, bool *connectedToSegment) const
{
...
// Updating
void VoxelWeapon::SetWeaponTrailsParams(const Matrix4x4& originMatrix, float scale)
{
	if(m_loaded == false)
	{
		return;
...
// Rendering
void VoxelWeapon::Render(bool renderOutline, bool reflection, bool silhouette, const Colour& OutlineColour)
{
