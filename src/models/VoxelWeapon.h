...
public:
	...
	void SetRenderOffset(const vec3& offset);
	const vec3& GetRenderOffset() const;
	...
	void GetLightParams(int lightIndex, unsigned int *lightId, vec3 *position, float *radius, float *diffuseMultiplier, Colour *colour, bool *connectedToSegment) const;

	// Particle effects
	int GetNumParticleEffects() const;
	void SetParticleEffectId(int particleEffectIndex, unsigned int particleEffectId);
	void GetParticleEffectParams(int particleEffectIndex, unsigned int *particleEffectId, vec3 *position, string* name, bool *connectedToSegment) const;
	...
	// Updating
	void SetWeaponTrailsParams(const Matrix4x4& originMatrix, float scale);
